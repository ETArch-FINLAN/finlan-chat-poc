...
public class Chat extends JFrame{
		...
		join.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(workspace != null && !workspace.isEmpty()){
					fin.createWorkspace(workspace);
					if(fin.joinWorkspace(workspace)){
				...
				if(padrao.isSelected())
					...
					for (Enumeration en = wsGroup.getElements(); en.hasMoreElements();) {
						...
						if (b.getModel() == wsGroup.getSelection()){
							...
							if(fin.disjoinWorkspace(b.getText())){
								fin.deleteWorkspace(b.getText());
	...
	private void createSocketAndListeners(final String source) {
		...
		if(fin.open()){
			...
			fin.createWorkspace("Default");
			if(!fin.joinWorkspace("Default")){
			...
			this.addWindowListener( new WindowAdapter(){   
				...
				public void windowClosing(WindowEvent evt){
					if(JOptionPane.showConfirmDialog(Chat.this, "Deseja realmente sair?", "",
						...
						//fin.disjoinWorkspace("Default");
						//fin.deleteWorkspace("Default");
						//fin.unregister();
						fin.close();
						System.exit(0);
					}else
	...
	public static void main(String[] args) {
		if(args.length > 0)
			FinSocket.interfaceDefault = args[0];
